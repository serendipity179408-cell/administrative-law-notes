#!/usr/bin/env python3
"""
掃描 repository 根目錄下所有符合「NN.html」命名規則的筆記檔案，
自動重新產生 index.html 中 <!-- AUTO-GENERATED-LIST-START/END --> 之間的清單區塊。

命名規則（刻意只用純數字＋.html，避免中文檔名跨系統／跨瀏覽器的編碼不一致問題）：
  01.html
  02.html
  03.html
  ...
...
def collect_entries():
    ...
    for fname in sorted(os.listdir(ROOT)):
        ...
        if not m:
            print(f"[skip] 檔名不符合 NN.html 規則，略過：{fname}")
            continue
        num_str = m.group(1)
        ...
        title = extract_h1(content) or f"第{num_str}堂課"
